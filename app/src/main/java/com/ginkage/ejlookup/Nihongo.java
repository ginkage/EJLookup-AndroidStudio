...
class Nihongo {
    ...
    public static void Init(Resources res) {
        if (hashtab == null) {
            ...
            for (int i = 0; i < 65536; i++)
                if ((i >= 'A' && i <= 'Z') || (i >= 0x0410 && i <= 0x042F))
                    hashtab[i] = (char) (i + 0x20);
                else if (i == 0x0451 || i == 0x0401) hashtab[i] = 0x0435;
                else if (i == 0x040E || i == 0x045E) hashtab[i] = 0x0443;
                else if (i == 0x3000) hashtab[i] = 0x0020;
                else if (i >= 0x30A1 && i <= 0x30F4) hashtab[i] = (char) (i - 0x60);
                else if (i >= 0xFF01 && i <= 0xFF20) hashtab[i] = (char) (i - 0xFEE0);
                else if (i >= 0xFF21 && i <= 0xFF3A) hashtab[i] = (char) (i - 0xFEC0);
                else if (i >= 0xFF3B && i <= 0xFF5E) hashtab[i] = (char) (i - 0xFEE0);
                else hashtab[i] = (char) i;
        }

        if (kana == null) kana = readResource(res, R.raw.kanatab);

        if (roma == null) roma = readResource(res, R.raw.romatab);
    }

    private static boolean Jaiueoy(char c) {
        return (c >= 0x3041 && c <= 0x304A)
                || (c >= 0x30A1 && c <= 0x30AA)
                || (c >= 0x3083 && c <= 0x3088)
                || (c >= 0x30E3 && c <= 0x30E8);
    }
    ...
    public static String Kanate(char[] text) {
        ...
        for (pb = 0; pb < text.length; pb++) {
            ...
            if (pb < text.length && ((pls = findsub(text, pb)) >= 0)) {
                ...
                pk = prs;
            } else if (tolower(text[pb]) == 'n' || tolower(text[pb]) == 'm') kanabuf[pk++] = 0x3093;
            else {
                char[] tmp = new char[4];
                pls = -1;
    ...
    public static String Romanate(char[] text, int begin, int end) {
        ...
        for (pb = begin; pb <= end; pb++) {
            if ((text[pb] >= 0x3041 && text[pb] <= 0x3094)
                    || (text[pb] >= 0x30A1 && text[pb] <= 0x30FC)) {
                if (text[pb] == 0x3063 || text[pb] == 0x30C3) {
                    if (pb + 1 <= end
                            && ((text[pb + 1] >= 0x3041 && text[pb + 1] <= 0x3094)
                ...
                for (pkana = kana.length - 1; pkana >= 0; pkana--) {
                    for (pk = 0, pi = pb;
                            pi <= end && pk < kana[pkana].length && kana[pkana][pk] != '=';
                            pk++, pi++)
                        if (kana[pkana][pk] != text[pi] && kana[pkana][pk] != (text[pi] - 0x60))
                            break;
                    ...
                    if (kana[pkana][pk] == '=') {
                        ...
                        out.append(kana[pkana], ps, kana[pkana].length - ps);
        ...
        for (unibuf = p = 0; p < buffer.length && buffer[p] != 0; p++) {
            if (buffer[p] >= 0xFF61 && buffer[p] <= 0xFF9F) {
                switch (buffer[p]) {
                    ...
                    case 0xFF6A:
                    case 0xFF6B:
                        buffer[p] = (char) ((buffer[p] - 0xFF67) * 2 + 0x30A1);
                        break;

                    case 0xFF6C:
                    case 0xFF6D:
                    case 0xFF6E:
                        buffer[p] = (char) ((buffer[p] - 0xFF6C) * 2 + 0x30E3);
                        break;
                    ...
                    case 0xFF74:
                    case 0xFF75:
                        buffer[p] = (char) ((buffer[p] - 0xFF71) * 2 + 0x30A2);
                        break;
                    ...
                    case 0xFF80:
                    case 0xFF81:
                        buffer[p] = (char) ((buffer[p] - 0xFF76) * 2 + 0x30AB);
                        break;

                    case 0xFF82:
                    case 0xFF83:
                    case 0xFF84:
                        buffer[p] = (char) ((buffer[p] - 0xFF82) * 2 + 0x30C4);
                        break;
                    ...
                    case 0xFF88:
                    case 0xFF89:
                        buffer[p] = (char) ((buffer[p] - 0xFF85) + 0x30CA);
                        break;
                    ...
                    case 0xFF8D:
                    case 0xFF8E:
                        buffer[p] = (char) ((buffer[p] - 0xFF8A) * 3 + 0x30CF);
                        break;
                    ...
                    case 0xFF92:
                    case 0xFF93:
                        buffer[p] = (char) ((buffer[p] - 0xFF8F) + 0x30DE);
                        break;

                    case 0xFF94:
                    case 0xFF95:
                    case 0xFF96:
                        buffer[p] = (char) ((buffer[p] - 0xFF94) * 2 + 0x30E4);
                        break;
                    ...
                    case 0xFF9A:
                    case 0xFF9B:
                        buffer[p] = (char) ((buffer[p] - 0xFF97) + 0x30E9);
                        break;
                    ...
                    case 0xFF9E:
                        if (unibuf > 0) buffer[unibuf - 1] += 1;
                        break;
                    case 0xFF9F:
                        if (unibuf > 0) buffer[unibuf - 1] += 2;
                        break;
                }
                }
