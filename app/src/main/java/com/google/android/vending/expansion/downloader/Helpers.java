...
public class Helpers {
    ...
    public static String getDownloadProgressString(long overallProgress, long overallTotal) {
        ...
        return String.format("%.2f", (float) overallProgress / (1024.0f * 1024.0f))
                + "MB /"
                + String.format("%.2f", (float) overallTotal / (1024.0f * 1024.0f))
                + "MB";
    }
     ...
     * @return
     */
    public static String getDownloadProgressStringNotification(
            long overallProgress, long overallTotal) {
        if (overallTotal == 0) {
            if (Constants.LOGVV) {
                Log.e(Constants.TAG, "Notification called when total is zero");
            }
    ...
    public static boolean doesFileExist(
            Context c, String fileName, long fileSize, boolean deleteFileOnMismatch) {
     ...
     * IDownloaderClient#onDownloadStateChanged} callback into usable strings. This is useful if
     * using the state strings built into the library to display user messages.
     *
