...
/**
 * Performs the background downloads requested by applications that use the Downloads provider. This
 * service does not run as a foreground task, so Android may kill it off at will, but it will try to
 * restart itself if it can. Note that Android by default will kill off any process that has an open
 * file handle on the shared (SD Card) partition if the partition is unmounted.
 */
public abstract class DownloaderService extends CustomIntentService implements IDownloaderService {
    ...
    public static final String ACTION_DOWNLOAD_COMPLETE =
            "lvldownloader.intent.action.DOWNLOAD_COMPLETE";
    ...
    public static final String ACTION_DOWNLOAD_STATUS =
            "lvldownloader.intent.action.DOWNLOAD_STATUS";
    ...
    /**
     * This download is waiting for a Wi-Fi connection to proceed or for permission to download over
     * cellular.
     */
    ...
    /**
     * This download has successfully completed. Warning: there might be other status values that
    ...
    /**
     * This download has completed with an error. Warning: there will be other status values that
     * indicate errors in the future. Use isStatusError() to capture the entire category.
     */
    public static final int STATUS_UNKNOWN_ERROR = 491;

    /**
     * This download couldn't be completed because of a storage issue. Typically, that's because the
     * filesystem is missing or full. Use the more specific {@link #STATUS_INSUFFICIENT_SPACE_ERROR}
    ...
    /**
     * This download couldn't be completed because of an HTTP redirect response that the download
    ...
    /**
     * This download couldn't be completed because of an error receiving or processing data at the
     * HTTP level.
     *
     * @hide
     */
    ...
    /**
     * This download couldn't be completed due to insufficient storage space. Typically, this is
    ...
    /**
     * This download couldn't be completed because no external storage device was found. Typically,
    ...
    /**
     * This download is visible and shows in the notifications while in progress and after
     * completion.
     *
     * @hide
     */
    ...
    /**
     * Bit flag for {@link #setAllowedNetworkTypes} corresponding to {@link
    ...
    private final Messenger mServiceMessenger = mServiceStub.getMessenger();
    ...
    /**
     * Updates the network type based upon the type and subtype returned from the connectivity
     * manager. Subtype is only used for cellular signals.
     *
     * @param type
     * @param subType
     */
    ...
    public static int startDownloadServiceIfRequired(
            Context context, Intent intent, Class<?> serviceClass) throws NameNotFoundException {
    ...
    public static int startDownloadServiceIfRequired(
            Context context, PendingIntent pendingIntent, Class<?> serviceClass)
    ...
    /**
     * Starts the download if necessary. This function starts a flow that does ` many things. 1)
     * Checks to see if the APK version has been checked and the metadata database updated 2) If the
     * APK version does not match, checks the new LVL status to see if a new download is required 3)
     * If the APK version does match, then checks to see if the download(s) have been completed 4)
     * If the downloads have been completed, returns NO_DOWNLOAD_REQUIRED The idea is that this can
     * be called during the startup of an application to quickly ascertain if the application needs
     * to wait to hear about any updated APK expansion files. Note that this does mean that the
     * application MUST be run for the first time with a network connection, even if Market delivers
     * all of the files.
     *
     * @param context
     * @param thisIntent
     * @return true if the app should wait for more guidance from the downloader, false if the app
     *     can continue
     * @throws NameNotFoundException
     */
    public static int startDownloadServiceIfRequired(
            Context context, PendingIntent pendingIntent, String classPackage, String className)
            throws NameNotFoundException {
    ...
    private class LVLRunnable implements Runnable {
        ...
        public void run() {
            ...
            final APKExpansionPolicy aep =
                    new APKExpansionPolicy(
                            mContext,
            ...
            final LicenseChecker checker =
                    new LicenseChecker(
                            mContext, aep, getPublicKey() // Your public licensing key.
                            );
            checker.checkAccess(
                            ...
                            try {
                                ...
                                if (count != 0) {
                                    for (int i = 0; i < count; i++) {
                                        ...
                                        if (null != currentFileName) {
                                            ...
                                            if (handleFileUpdated(
                                                    db, i, currentFileName, fileSize)) {
                                                status |= -1;
                                                di.resetDownload();
                                                ...
                                                // the database
                                                DownloadInfo dbdi =
                                                        db.getDownloadInfoByFileName(di.mFileName);
                                                if (null == dbdi) {
                                ...
                                try {
                                    ...
                                    switch (startDownloadServiceIfRequired(
                                            mContext, mPendingIntent, serviceClass)) {
                                        case NO_DOWNLOAD_REQUIRED:
                                            mNotification.onDownloadStateChanged(
    ...
    /**
     * The APK has been updated and a filename has been sent down from the Market call. If the file
     * has the same name as the previous file, we do nothing as the file is guaranteed to be the
     * same. If the file does not have the same name, we download it if it hasn't already been
     * delivered by Market.
     *
    ...
    private void scheduleAlarm(long wakeUp) {
        ...
        alarms.set(AlarmManager.RTC_WAKEUP, System.currentTimeMillis() + wakeUp, mAlarmIntent);
    }
    ...
    /**
     * We use this to track network state, such as when WiFi, Cellular, etc. is enabled when
     * downloads are paused or in progress.
     */
    ...
    /**
     * This is the main thread for the Downloader. This thread is responsible for queuing up
     * downloads and other goodness.
     */
    @Override
    ...
    public int getNetworkAvailabilityState(DownloadsDB db) {
        if (mIsConnected) {
            if (!mIsCellularConnection) return NETWORK_OK;
            int flags = db.mFlags;
            if (mIsRoaming) return NETWORK_CANNOT_USE_ROAMING;
            if (0 != (flags & FLAGS_DOWNLOAD_OVER_CELLULAR)) {
                return NETWORK_OK;
            } else {
    ...
    public String generateSaveFile(String filename, long filesize) throws GenerateSaveFileError {
        ...
        if (!Helpers.isExternalMediaMounted()) {
            ...
            throw new GenerateSaveFileError(
                    STATUS_DEVICE_NOT_FOUND_ERROR, "external media is not yet mounted");
        }
        if (expPath.exists()) {
            Log.d(Constants.TAG, "File already exists: " + path);
            throw new GenerateSaveFileError(
                    STATUS_FILE_ALREADY_EXISTS_ERROR, "requested destination file already exists");
        }
        if (Helpers.getAvailableBytes(Helpers.getFilesystemRoot(path)) < filesize) {
            throw new GenerateSaveFileError(
                    STATUS_INSUFFICIENT_SPACE_ERROR, "insufficient space on external storage");
        }
        return path;
    }

    /**
     * @return a non-localized string appropriate for logging corresponding to one of the NETWORK_*
     *     constants.
     */
    ...
    public void notifyUpdateBytes(long totalBytesSoFar) {
        ...
        mNotification.onDownloadProgress(
                new DownloadProgressInfo(
                        mTotalLength, totalBytesSoFar, timeRemaining, mAverageDownloadSpeed));
    }
